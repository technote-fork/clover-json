...
const unpackage = (projects): classCov[] => classDetailsFromProjects(projects).map(detail => {
...
export const parseContent = (xml: string): Promise<classCov[]> => new Promise((resolve, reject): void => {
...
export const parseFile = async(filePath: string): Promise<classCov[]> => await parseContent(await promisify(fs.readFile)(filePath, 'utf-8'));
